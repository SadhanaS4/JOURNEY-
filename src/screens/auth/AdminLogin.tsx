...
export function AdminLogin() {
  const { loginAdmin, goBack } = useApp()
  ...
  return (
    <div className="min-h-screen px-6 pt-8 pb-12">
      ...
      <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }}>
        <p className="text-[11px] font-medium tracking-[0.2em] text-nexa/70 uppercase mb-2">Authority Portal</p>
        <h2 className="font-display text-2xl font-semibold text-cream mb-1">Admin Access</h2>
        <p className="text-sm text-cream/40 mb-8 font-light">Railway command center</p>
        ...
        <div className="mt-8 glass p-4 rounded-2xl">
          ...
          <p className="text-xs text-cream/50 font-mono">admin / admin123</p>
        </div>
      </motion.div>
    </div>
  )
